...
（GUI操作系统）这就是操作系统，那么什么是GUI操作系统呢？能让你用鼠标点击，用屏幕呈现图形的操作系统就是GUI系统。现在市面上主要有两种这样的电脑系统，一种是`Windows系列`，其以出色的兼容性和丰富的软件市场有着很高的使用率，目前最新的系统是`Windows 11`；另一种是`macOS系列`，其以更人性化和Apple的服务得到一部分群体的青睐，当然`Harmony OS`系统也来了，相信大家都很期待它能越来越强大和好用。
...
在`macOS`中，你需要对`Finder（访达）`做一些设置才能看到文件的路径。

在`Finder`的 `偏好设置 - 高级` 中勾选`Show all filename extensions`，打开显示所有文件扩展名的选项。

![](assets/17386573877568.jpg){width="300"}


在Finder的 `偏好设置 - 边栏` 中`Favorites(个人收藏)`类里勾选像房子一样图标的家目录，我的是`apple`也就是用户名，在`Locations（位置）`勾选硬盘`Hard Disk（硬盘）`。

![](assets/17386578037635.jpg){width="300"}


![](assets/17386577770123.jpg){width="300"}
...
![](assets/17386580786560.jpg){width="300"}


![](assets/17386581609973.jpg)
{width="300"}
...
比如我桌面上的待办事项：`/Users/apple/Desktop/TODO.txt`
...
![](assets/17386582017632.jpg){width="300"}
...
```
//Print the date and time represented by seconds, where seconds is the number of seconds since the Epoch (00:00:00 UTC, January 1, 1970)
-r <seconds> 
...
$ /Users/apple/desktop/CODE/SayHello
…
$ SayHello
...
```shell
vim # 好用的命令行文本编辑器，使用这个命令还可以直接新建文件
...
```shell
history       //查看历史输入的命令行指令
history | grep "mkdir"   //查看历史指令并通过管道给到grep搜索其中带有mkdir的命令行指令
```
...
$ echo "abc123def" | grep "[a-z]"
abc123def（abc def得到高亮）
...
在空格之前加入`\`转义；或用 `""` `''` 包裹含有空格的文件和文件夹，最好就是文件命名的时候干脆不包含空格或者使用`_`和`-`之类的符号隔开。
...
![](assets/17386583025328.jpg){width="300"}
...
![](assets/17386647855962.jpg){width="300"}
...
![](assets/17386648816871.jpg){width="300"}
...
![](assets/17386650051566.jpg){width="300"}
