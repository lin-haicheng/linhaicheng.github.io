...
> 扩展了增加找到最小数，如果只是找最大值的话，第一个if语句就可以了。  
> 新思路**变量的初始值未必一定要定义为0，可以灵活修改。**
...
推导过程：
![4311737258224_.pic](assets/4311737258224_.pic.jpg)
...
> 2、调用函数其实相当于把这个函数运行了一遍，所以他里面的其他代码如cout之类的打印也会运行，不是只会返回一个值。
